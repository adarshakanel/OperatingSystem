...
struct Task {int64_t f;};
Task task[256];
int64_t n;
std::atomic<int64_t> min, min_index;
int64_t sum ;
int64_t multiplication;
//finder will take in arg int64_t index and will return the number of threads that are done thier calculation before it
// this is to check if this factor is indeed the smallest one
...
//factor takes an argument index void * ind
// it then uses the thread [index ind] to run on each standard input
// it will continue trying to find a factor, until the smallest one is one, from where it will cancel all other threads, and return that smallest factor
int64_t factor(void * ind){
	...
	if(done == false){
		if( n <= 3) {
			...
			done = true;	// make done true, so other threads can cancel
			return min;
		} // 2 and 3 are primes
		else if( n % 2 == 0){
...
// get_smallest_divisor reads in all positive integers in range (2 .. 2^63-1) from standard input
// for each number on input:
//   determines the smallest factor of the composite number of standard input
// returns the total sum of each of the smallest factors
//
...
//sum_factors takes in arg n_thread
// returns the sum of the smallest factors of the integers in a given file using n_thread number of threads
